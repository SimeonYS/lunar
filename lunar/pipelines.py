...
class LunarPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('lunar.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `lunar`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM lunar WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO lunar (date, title, link, content)"
